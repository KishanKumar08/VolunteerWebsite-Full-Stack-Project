...
signUpForm.addEventListener("submit", async (e) => {
    ...
    const address = document.getElementById("address-sign-up").value;
    const mission = document.getElementById("mission-sign-up").value;

    try{
        const res = await fetch("http://127.0.0.1:8000/main/organization/register/",{
            method:"POST",
            headers:{
                "Content-Type":"application/json"
            },
            body:JSON.stringify(
                {name,email,password,address,mission}
            )
        })
